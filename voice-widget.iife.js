(()=>{var h=class{constructor(){this.audioChunks=[],this.selectedDeviceId=null,this.stream=null,this.audioContext=null,this.sourceSampleRate=null,this.targetSampleRate=16e3,this.bufferedData=[],this._workletModuleLoaded=!1}async start(e){this.selectedDeviceId=e,this.stream=await navigator.mediaDevices.getUserMedia({audio:{deviceId:{exact:e}}}),this.audioContext=new AudioContext,this.sourceSampleRate=this.audioContext.sampleRate;let i=this.audioContext.createMediaStreamSource(this.stream),o=new AudioWorkletNode(await this._getAudioWorkletContext(),"capture-processor");o.port.onmessage=t=>{let n=t.data;this.bufferedData.push(...n)},i.connect(o).connect(this.audioContext.destination),this.processor=o}async stop(){return new Promise(async e=>{this.processor.disconnect(),this.stream.getTracks().forEach(n=>n.stop()),await this.audioContext.close();let i=await this._resampleToTarget(this.bufferedData,this.sourceSampleRate,this.targetSampleRate),o=this._convertFloat32ToInt16(i),t=this._encodeWAV(o,this.targetSampleRate);e(t)})}async _getAudioWorkletContext(){if(!this._workletModuleLoaded){let e=new Blob([`
        ...
        `],{type:"application/javascript"}),i=URL.createObjectURL(e);await this.audioContext.audioWorklet.addModule(i),this._workletModuleLoaded=!0}return this.audioContext}async _resampleToTarget(e,i,o){if(i===o)return Float32Array.from(e);let t=new OfflineAudioContext(1,e.length*o/i,o),n=t.createBuffer(1,e.length,i);n.copyToChannel(Float32Array.from(e),0);let r=t.createBufferSource();return r.buffer=n,r.connect(t.destination),r.start(),(await t.startRendering()).getChannelData(0)}_convertFloat32ToInt16(e){let i=new Int16Array(e.length);for(let o=0;o<e.length;o++)i[o]=Math.max(-32768,Math.min(32767,e[o]*32767));return i}_encodeWAV(e,i){let o=new ArrayBuffer(44+e.length*2),t=new DataView(o),n=(r,l,c)=>{for(let a=0;a<c.length;a++)r.setUint8(l+a,c.charCodeAt(a))};n(t,0,"RIFF"),t.setUint32(4,36+e.length*2,!0),n(t,8,"WAVE"),n(t,12,"fmt "),t.setUint32(16,16,!0),t.setUint16(20,1,!0),t.setUint16(22,1,!0),t.setUint32(24,i,!0),t.setUint32(28,i*2,!0),t.setUint16(32,2,!0),t.setUint16(34,16,!0),n(t,36,"data"),t.setUint32(40,e.length*2,!0);for(let r=0;r<e.length;r++)t.setInt16(44+r*2,e[r],!0);return new Blob([t],{type:"audio/wav"})}};var d={websocketUrl:"wss://ksamuwry9l.execute-api.us-east-1.amazonaws.com/production/",apiGatewayUploadUrl:"https://3c8t1k5p92.execute-api.us-east-1.amazonaws.com/stage/upload",tokenEndpoint:"https://q19mkers91.execute-api.us-east-1.amazonaws.com/dev/widget/token",integrationMode:"form-fill",debugMode:!1};async function m(s,e,i){if(!e)return console.error("No FormId configured"),null;console.log("Obtaining token...");let o=await g(e);if(console.log("Token:",o),!o)throw new Error("Unable to obtain token");let{ready:t,message:n}=q(i,"standalone");if(await t,!await w(s,e,i,o))throw new Error("Unable to upload audio");return await n}async function g(s){let e=await fetch(`${d.tokenEndpoint}?formId=${s}`);if(!e.ok)throw new Error("Unable to fetch token");return(await e.json()).token}async function w(s,e,i,o){let t=`${i}.wav`,n=`${i}.json`,r={app_template:" ",formId:e,param3:"parameter3"},l=new URLSearchParams({sessionId:i,wav_filename:t,metadata_filename:n,metadata_content:btoa(JSON.stringify(r))}),c=`${d.apiGatewayUploadUrl}?${l.toString()}`;console.log("Posting to",c);let a=await fetch(c,{method:"POST",headers:{Authorization:`Bearer ${o}`,"Content-Type":"audio/wav"},body:s});return console.log(a),a.ok?(console.log("Upload successful"),!0):(console.error("Upload failed with status:",a.status),null)}function q(s,e="standalone",i=3e4){let o=`${d.websocketUrl}?sessionId=${s}&role=${e}`;console.log("Connecting WebSocket:",o);let t=new WebSocket(o),n=new Promise((l,c)=>{t.onopen=()=>{console.log("WebSocket connected"),l()},t.onerror=a=>c(a)}),r=new Promise((l,c)=>{let a=setTimeout(()=>{t.close(),console.log("timeout"),c(new Error(`Timeout: No WebSocket message received in ${i} ms`))},i);t.onmessage=u=>{clearTimeout(a),console.log("WebSocket message received:",u.data),t.close(),l(JSON.parse(u.data))},t.onerror=u=>{clearTimeout(a),t.close(),c(new Error(`WebSocket error: ${u.message}`))},t.onclose=()=>{console.log("WebSocket closed")}});return{ws:t,ready:n,message:r}}function f(s,e=null){d.integrationMode==="debug"?b(s):d.integrationMode==="form-fill"&&S(s,e)}function b(s){let e=document.createElement("pre");e.textContent=JSON.stringify(s,null,2),document.body.appendChild(e)}function S(s,e){let i=e?document.getElementById(e):document;Object.entries(s).forEach(([o,t])=>{let n=i.querySelector(`#${o}`);n&&(n.value=t)})}var p=class{constructor(e=null,i=null){console.log("in Constructor ..."),this.formId=e,this.scriptElement=i,this.recorder=new h,this.sessionId=this.generateSessionId(),this.selectedDeviceId=null,this.injectStyles(),this.createDOM(),this.setupUI()}injectStyles(){if(document.getElementById("qiqi-style"))return;let e=document.createElement("style");e.id="qiqi-style",e.textContent=`
      .qiqi-widget-inline {
        display: inline-flex;
