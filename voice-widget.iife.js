(()=>{var d=class{constructor(){this.mediaRecorder=null,this.audioChunks=[],this.selectedDeviceId=null,this.stream=null}async start(e){this.selectedDeviceId=e,this.stream=await navigator.mediaDevices.getUserMedia({audio:{deviceId:{exact:e}}}),this.mediaRecorder=new MediaRecorder(this.stream),this.mediaRecorder.ondataavailable=t=>{this.audioChunks.push(t.data)},this.mediaRecorder.start()}async stop(){return new Promise(e=>{this.mediaRecorder.onstop=async()=>{let t=new Blob(this.audioChunks,{type:"audio/webm"});this.stream.getTracks().forEach(o=>o.stop()),e(t)},this.mediaRecorder.stop()})}};var c={websocketUrl:"wss://ksamuwry9l.execute-api.us-east-1.amazonaws.com/production/",apiGatewayUploadUrl:"https://3c8t1k5p92.execute-api.us-east-1.amazonaws.com/stage/upload",tokenEndpoint:"https://q19mkers91.execute-api.us-east-1.amazonaws.com/dev/widget/token",integrationMode:"form-fill",debugMode:!1};async function u(i,e,t){if(!e)return console.error("No FormId configured"),null;console.log("obtaining Token ...");let o=await g(e);if(console.log(o),!o)throw new Error("Unable to obtainToken");console.log("obtaining Transcript** ...");let n=await f(i,e,t,o);if(!response)throw new Error("Unable to obtainFormTranscription");return n}async function g(i){i||console.error("No FormId configured");let e=await fetch(`${c.tokenEndpoint}?formId=${i}`);if(!e.ok)throw new Error("Unable to fetch token");return(await e.json()).token}async function f(i,e,t,o){let n="standalone",r=`${t}.wav`,a=`${t}.json`,l={app_template:" ",formId:e,param3:"parameter3"},s=new URLSearchParams({sessionId:t,wav_filename:r,metadata_filename:a,metadata_content:btoa(JSON.stringify(l))});console.log("Calling waitForWebSocketMessageAfterPostxxxx"),console.log(`${c.websocketUrl}?sessionId=${t}&role=${n}`),console.log(`${c.apiGatewayUploadUrl}?${s.toString()}`),await w(`${c.websocketUrl}?sessionId=${t}&role=${n}`,`${c.apiGatewayUploadUrl}?${s.toString()}`,i,7e3).then(m=>{console.log("Got WebSocket response:",m)}).catch(m=>{console.error("Error:",m.message)})}async function w(i,e,t,o=5e3){return console.log("inwait ..."),console.log(i),console.log(e),new Promise((n,r)=>{let a=new WebSocket(i),l;a.onopen=async()=>{console.log("WebSocket connected. Sending POST to REST API..."),l=setTimeout(()=>{a.close(),r(new Error(`Timeout: No WebSocket message received in ${o} ms`))},o);try{let s=await fetch(e,{method:"POST",headers:{Authorization:`Bearer ${token}`,"Content-Type":"audio/wav"},body:t});s.ok?console.log("REST API call succeeded"):(clearTimeout(l),a.close(),r(new Error(`REST API call failed with status ${s.status}`)))}catch(s){clearTimeout(l),a.close(),r(new Error(`REST API call error: ${s.message}`))}},a.onmessage=s=>{clearTimeout(l),console.log("WebSocket message received:",s.data),a.close(),n(s.data)},a.onerror=s=>{clearTimeout(l),a.close(),r(new Error(`WebSocket error: ${s.message}`))},a.onclose=()=>{console.log("WebSocket closed")}})}function p(i,e=null){c.integrationMode==="debug"?b(i):c.integrationMode==="form-fill"&&q(i,e)}function b(i){let e=document.createElement("pre");e.textContent=JSON.stringify(i,null,2),document.body.appendChild(e)}function q(i,e){let t=e?document.getElementById(e):document;Object.entries(i).forEach(([o,n])=>{let r=t.querySelector(`#${o}`);r&&(r.value=n)})}var h=class{constructor(e=null,t=null){console.log("in Constructor ..."),this.formId=e,this.scriptElement=t,this.recorder=new d,this.sessionId=this.generateSessionId(),this.selectedDeviceId=null,this.injectStyles(),this.createDOM(),this.setupUI()}injectStyles(){if(document.getElementById("qiqi-style"))return;let e=document.createElement("style");e.id="qiqi-style",e.textContent=`
      .qiqi-widget-inline {
        display: inline-flex;
