(()=>{var h=class{constructor(){this.audioChunks=[],this.selectedDeviceId=null,this.stream=null,this.audioContext=null,this.sourceSampleRate=null,this.targetSampleRate=16e3,this.bufferedData=[],this._workletModuleLoaded=!1}async start(e){console.log("\u{1F501} Recorder start"),this.bufferedData=[],this.selectedDeviceId=e,this.stream=await navigator.mediaDevices.getUserMedia({audio:{deviceId:{exact:e}}}),this.audioContext=new AudioContext,this.sourceSampleRate=this.audioContext.sampleRate,console.log("\u{1F399}\uFE0F Audio context sample rate:",this.sourceSampleRate),await this._getAudioWorkletContext();let o=new AudioWorkletNode(this.audioContext,"capture-processor");o.port.onmessage=t=>{let s=t.data;this.bufferedData.push(...s),console.log("\u{1F3A7} Received chunk:",s.length)},this.audioContext.createMediaStreamSource(this.stream).connect(o).connect(this.audioContext.destination),this.processor=o}async stop(){return new Promise(async e=>{this.processor&&(this.processor.disconnect(),this.processor=null),this.stream&&(this.stream.getTracks().forEach(s=>s.stop()),this.stream=null),this.audioContext&&this.audioContext.state!=="closed"&&await this.audioContext.close(),this.audioContext=null,this._workletModuleLoaded=!1,console.log("\u{1F9F5} Buffered frames:",this.bufferedData.length);let o=await this._resampleToTarget(this.bufferedData,this.sourceSampleRate,this.targetSampleRate),i=this._convertFloat32ToInt16(o),t=this._encodeWAV(i,this.targetSampleRate);e(t)})}async _getAudioWorkletContext(){if(!this._workletModuleLoaded){console.log("\u{1F39B}\uFE0F Loading audio worklet module...");let e=new Blob([`
        ...
        `],{type:"application/javascript"}),o=URL.createObjectURL(e);await this.audioContext.audioWorklet.addModule(o),this._workletModuleLoaded=!0}return this.audioContext}async _resampleToTarget(e,o,i){if(o===i)return Float32Array.from(e);let t=new OfflineAudioContext(1,e.length*i/o,i),s=t.createBuffer(1,e.length,o);s.copyToChannel(Float32Array.from(e),0);let r=t.createBufferSource();return r.buffer=s,r.connect(t.destination),r.start(),(await t.startRendering()).getChannelData(0)}_convertFloat32ToInt16(e){let o=new Int16Array(e.length);for(let i=0;i<e.length;i++)o[i]=Math.max(-32768,Math.min(32767,e[i]*32767));return o}_encodeWAV(e,o){let i=new ArrayBuffer(44+e.length*2),t=new DataView(i),s=(r,c,l)=>{for(let a=0;a<l.length;a++)r.setUint8(c+a,l.charCodeAt(a))};s(t,0,"RIFF"),t.setUint32(4,36+e.length*2,!0),s(t,8,"WAVE"),s(t,12,"fmt "),t.setUint32(16,16,!0),t.setUint16(20,1,!0),t.setUint16(22,1,!0),t.setUint32(24,o,!0),t.setUint32(28,o*2,!0),t.setUint16(32,2,!0),t.setUint16(34,16,!0),s(t,36,"data"),t.setUint32(40,e.length*2,!0);for(let r=0;r<e.length;r++)t.setInt16(44+r*2,e[r],!0);return new Blob([t],{type:"audio/wav"})}};var d={websocketUrl:"wss://ksamuwry9l.execute-api.us-east-1.amazonaws.com/production/",apiGatewayUploadUrl:"https://3c8t1k5p92.execute-api.us-east-1.amazonaws.com/stage/upload",tokenEndpoint:"https://q19mkers91.execute-api.us-east-1.amazonaws.com/dev/widget/token",integrationMode:"form-fill",debugMode:!1};async function m(n,e,o){if(!e)return console.error("No FormId configured"),null;console.log("Obtaining token...");let i=await g(e);if(console.log("Token:",i),!i)throw new Error("Unable to obtain token");let{ready:t,message:s}=q(o,"standalone");if(await t,!await w(n,e,o,i))throw new Error("Unable to upload audio");return await s}async function g(n){let e=await fetch(`${d.tokenEndpoint}?formId=${n}`);if(!e.ok)throw new Error("Unable to fetch token");return(await e.json()).token}async function w(n,e,o,i){let t=`${o}.wav`,s=`${o}.json`,r={app_template:" ",formId:e,param3:"parameter3"},c=new URLSearchParams({sessionId:o,wav_filename:t,metadata_filename:s,metadata_content:btoa(JSON.stringify(r))}),l=`${d.apiGatewayUploadUrl}?${c.toString()}`;console.log("Posting to",l);let a=await fetch(l,{method:"POST",headers:{Authorization:`Bearer ${i}`,"Content-Type":"audio/wav"},body:n});return console.log(a),a.ok?(console.log("Upload successful"),!0):(console.error("Upload failed with status:",a.status),null)}function q(n,e="standalone",o=3e4){let i=`${d.websocketUrl}?sessionId=${n}&role=${e}`;console.log("Connecting WebSocket:",i);let t=new WebSocket(i),s=new Promise((c,l)=>{t.onopen=()=>{console.log("WebSocket connected"),c()},t.onerror=a=>l(a)}),r=new Promise((c,l)=>{let a=setTimeout(()=>{t.close(),console.log("timeout"),l(new Error(`Timeout: No WebSocket message received in ${o} ms`))},o);t.onmessage=u=>{clearTimeout(a),console.log("WebSocket message received:",u.data),t.close(),c(JSON.parse(u.data))},t.onerror=u=>{clearTimeout(a),t.close(),l(new Error(`WebSocket error: ${u.message}`))},t.onclose=()=>{console.log("WebSocket closed")}});return{ws:t,ready:s,message:r}}function f(n,e=null){d.integrationMode==="debug"?b(n):d.integrationMode==="form-fill"&&(console.log("In handleResonse",n),y(n,e))}function b(n){let e=document.createElement("pre");e.textContent=JSON.stringify(n,null,2),document.body.appendChild(e)}function y(n,e){Object.keys(n).length===1&&typeof Object.values(n)[0]=="object"&&(n=Object.values(n)[0]);let o=new Set(["sessionId","app_template"]);for(let[i,t]of Object.entries(n)){if(o.has(i))continue;console.log("In fillform",i);let s=i.trim().toLowerCase().replace(/\s+/g,"_"),r=document.getElementById(i)||document.getElementById(s);if(console.log("in formfill input",r),r)this._fillInput(r,t,i);else{let c=document.querySelectorAll(`input[type="radio"][name="${i}"]`);c.length>0?this._fillRadioGroup(c,t,i):console.warn(`\u{1F50D} Field not found in DOM for key: "${i}"`)}}}var p=class{constructor(e=null,o=null){console.log("in Constructor ..."),this.formId=e,this.scriptElement=o,this.recorder=new h,this.sessionId=this.generateSessionId(),this.selectedDeviceId=null,this.injectStyles(),this.createDOM(),this.setupUI()}injectStyles(){if(document.getElementById("qiqi-style"))return;let e=document.createElement("style");e.id="qiqi-style",e.textContent=`
      .qiqi-widget-inline {
        display: inline-flex;
