(()=>{var h=class{constructor(){this.audioChunks=[],this.selectedDeviceId=null,this.stream=null,this.audioContext=null,this.sourceSampleRate=null,this.targetSampleRate=16e3,this.bufferedData=[],this._workletModuleLoaded=!1}async start(e){console.log("\u{1F501} Recorder start"),this.bufferedData=[],this.selectedDeviceId=e,this.stream=await navigator.mediaDevices.getUserMedia({audio:{deviceId:{exact:e}}}),this.audioContext=new AudioContext,this.sourceSampleRate=this.audioContext.sampleRate,console.log("\u{1F399}\uFE0F Audio context sample rate:",this.sourceSampleRate),await this._getAudioWorkletContext();let o=new AudioWorkletNode(this.audioContext,"capture-processor");o.port.onmessage=t=>{let i=t.data;this.bufferedData.push(...i),console.log("\u{1F3A7} Received chunk:",i.length)},this.audioContext.createMediaStreamSource(this.stream).connect(o).connect(this.audioContext.destination),this.processor=o}async stop(){return new Promise(async e=>{this.processor&&(this.processor.disconnect(),this.processor=null),this.stream&&(this.stream.getTracks().forEach(i=>i.stop()),this.stream=null),this.audioContext&&this.audioContext.state!=="closed"&&await this.audioContext.close(),this.audioContext=null,this._workletModuleLoaded=!1,console.log("\u{1F9F5} Buffered frames:",this.bufferedData.length);let o=await this._resampleToTarget(this.bufferedData,this.sourceSampleRate,this.targetSampleRate),n=this._convertFloat32ToInt16(o),t=this._encodeWAV(n,this.targetSampleRate);e(t)})}async _getAudioWorkletContext(){if(!this._workletModuleLoaded){console.log("\u{1F39B}\uFE0F Loading audio worklet module...");let e=new Blob([`
        ...
        `],{type:"application/javascript"}),o=URL.createObjectURL(e);await this.audioContext.audioWorklet.addModule(o),this._workletModuleLoaded=!0}return this.audioContext}async _resampleToTarget(e,o,n){if(o===n)return Float32Array.from(e);let t=new OfflineAudioContext(1,e.length*n/o,n),i=t.createBuffer(1,e.length,o);i.copyToChannel(Float32Array.from(e),0);let r=t.createBufferSource();return r.buffer=i,r.connect(t.destination),r.start(),(await t.startRendering()).getChannelData(0)}_convertFloat32ToInt16(e){let o=new Int16Array(e.length);for(let n=0;n<e.length;n++)o[n]=Math.max(-32768,Math.min(32767,e[n]*32767));return o}_encodeWAV(e,o){let n=new ArrayBuffer(44+e.length*2),t=new DataView(n),i=(r,l,a)=>{for(let c=0;c<a.length;c++)r.setUint8(l+c,a.charCodeAt(c))};i(t,0,"RIFF"),t.setUint32(4,36+e.length*2,!0),i(t,8,"WAVE"),i(t,12,"fmt "),t.setUint32(16,16,!0),t.setUint16(20,1,!0),t.setUint16(22,1,!0),t.setUint32(24,o,!0),t.setUint32(28,o*2,!0),t.setUint16(32,2,!0),t.setUint16(34,16,!0),i(t,36,"data"),t.setUint32(40,e.length*2,!0);for(let r=0;r<e.length;r++)t.setInt16(44+r*2,e[r],!0);return new Blob([t],{type:"audio/wav"})}};var d={websocketUrl:"wss://ksamuwry9l.execute-api.us-east-1.amazonaws.com/production/",apiGatewayUploadUrl:"https://3c8t1k5p92.execute-api.us-east-1.amazonaws.com/stage/upload",tokenEndpoint:"https://q19mkers91.execute-api.us-east-1.amazonaws.com/dev/widget/token",integrationMode:"form-fill",debugMode:!1};async function m(s,e,o){if(!e)return console.error("No FormId configured"),null;console.log("Obtaining token...");let n=await g(e);if(console.log("Token:",n),!n)throw new Error("Unable to obtain token");let{ready:t,message:i}=q(o,"standalone");if(await t,!await w(s,e,o,n))throw new Error("Unable to upload audio");return await i}async function g(s){let e=await fetch(`${d.tokenEndpoint}?formId=${s}`);if(!e.ok)throw new Error("Unable to fetch token");return(await e.json()).token}async function w(s,e,o,n){let t=`${o}.wav`,i=`${o}.json`,r={app_template:" ",formId:e,param3:"parameter3"},l=new URLSearchParams({sessionId:o,wav_filename:t,metadata_filename:i,metadata_content:btoa(JSON.stringify(r))}),a=`${d.apiGatewayUploadUrl}?${l.toString()}`;console.log("Posting to",a);let c=await fetch(a,{method:"POST",headers:{Authorization:`Bearer ${n}`,"Content-Type":"audio/wav"},body:s});return console.log(c),c.ok?(console.log("Upload successful"),!0):(console.error("Upload failed with status:",c.status),null)}function q(s,e="standalone",o=3e4){let n=`${d.websocketUrl}?sessionId=${s}&role=${e}`;console.log("Connecting WebSocket:",n);let t=new WebSocket(n),i=new Promise((l,a)=>{t.onopen=()=>{console.log("WebSocket connected"),l()},t.onerror=c=>a(c)}),r=new Promise((l,a)=>{let c=setTimeout(()=>{t.close(),console.log("timeout"),a(new Error(`Timeout: No WebSocket message received in ${o} ms`))},o);t.onmessage=u=>{clearTimeout(c),console.log("WebSocket message received:",u.data),t.close(),l(JSON.parse(u.data))},t.onerror=u=>{clearTimeout(c),t.close(),a(new Error(`WebSocket error: ${u.message}`))},t.onclose=()=>{console.log("WebSocket closed")}});return{ws:t,ready:i,message:r}}function f(s,e=null){d.integrationMode==="debug"?b(s):d.integrationMode==="form-fill"&&(console.log("In handleResonse",s),y(s,e))}function b(s){let e=document.createElement("pre");e.textContent=JSON.stringify(s,null,2),document.body.appendChild(e)}function y(s,e){Object.keys(s).length===1&&typeof Object.values(s)[0]=="object"&&(s=Object.values(s)[0]),console.log("in formfill data.type is",s.type);let o=new Set(["sessionId","app_template"]);for(let[i,r]of Object.entries(s)){if(o.has(i))continue;console.log("In fillform key = ",i);let l=i.trim().toLowerCase().replace(/\s+/g,"_"),a=document.getElementById(i)||document.getElementById(l);if(console.log("in formfill input",a.type),a)n(a,r,i);else{let c=document.querySelectorAll(`input[type="radio"][name="${i}"]`);c.length>0?t(c,r,i):console.warn(`\u{1F50D} Field not found in DOM for key: "${i}"`)}}function n(i,r,l){if(i.type==="radio"){let a=document.querySelectorAll(`input[type="radio"][name="${i.name}"]`);this._fillRadioGroup(a,r,l)}else i.value=r||"",console.log(`\u2705 Filled "${l}" into #${i.id}`)}function t(i,r,l){let a=!1;i.forEach(c=>{c.value===r&&(c.checked=!0,a=!0)}),a?console.log(`\u{1F518} Selected radio "${r}" for "${l}"`):console.warn(`\u26A0\uFE0F No radio option matched value "${r}" for "${l}"`)}}var p=class{constructor(e=null,o=null){console.log("in Constructor ..."),this.formId=e,this.scriptElement=o,this.recorder=new h,this.sessionId=this.generateSessionId(),this.selectedDeviceId=null,this.injectStyles(),this.createDOM(),this.setupUI()}injectStyles(){if(document.getElementById("qiqi-style"))return;let e=document.createElement("style");e.id="qiqi-style",e.textContent=`
      .qiqi-widget-inline {
        display: inline-flex;
