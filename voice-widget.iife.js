(()=>{var h=class{constructor(){console.log("in recorder constructor"),this.audioChunks=[],this.selectedDeviceId=null,this.stream=null,this.audioContext=null,this.sourceSampleRate=null,this.targetSampleRate=16e3,this.bufferedData=[]}async start(e){console.log("recorder strated"),this.selectedDeviceId=e,this.stream=await navigator.mediaDevices.getUserMedia({audio:{deviceId:{exact:e}}}),this.audioContext=new AudioContext,this.sourceSampleRate=this.audioContext.sampleRate;let o=this.audioContext.createMediaStreamSource(this.stream),i=new AudioWorkletNode(await this._getAudioWorkletContext(),"capture-processor");i.port.onmessage=t=>{let n=t.data;this.bufferedData.push(...n)},o.connect(i).connect(this.audioContext.destination),this.processor=i}async stop(){return console.log("Recorder stopped"),new Promise(async e=>{this.processor.disconnect(),this.stream.getTracks().forEach(n=>n.stop()),await this.audioContext.close();let o=await this._resampleToTarget(this.bufferedData,this.sourceSampleRate,this.targetSampleRate),i=this._convertFloat32ToInt16(o),t=this._encodeWAV(i,this.targetSampleRate);e(t)})}async _getAudioWorkletContext(){if(!this.audioContext.audioWorklet.modules.includes("capture-processor")){let e=new Blob([`
